...
for (const k of [
  ...
  'MOONSHOT_API_KEY', 'ZHIPU_API_KEY', 'MIMO_API_KEY', 'NVIDIA_API_KEY',
...
// ── 场景 G：NVIDIA provider 被识别，未知模型回退到默认通用聊天模型 ──
{
  const { config, __internals } = await loadFresh({
    provider: 'nvidia',
    apiKey: 'nvapi-fake-key-1234567890abcdef',
    model: 'retired-nvidia-model',
  })
  const validIds = new Set(__internals.NVIDIA_MODELS.map(m => m.id))
  assert(config.needsActivation === false, 'G: NVIDIA provider 正常激活')
  assert(config.provider === 'nvidia', 'G: NVIDIA provider 正确')
  assert(config.model === 'openai/gpt-oss-120b', `G: NVIDIA 未知 model 回退默认（实得 ${config.model}）`)
  assert(validIds.has(config.model), 'G: NVIDIA 默认 model 在模型列表中')
}

// ── 场景 H：NVIDIA_API_KEY 环境变量可作为 Node 调试兜底来源 ──
{
  process.env.NVIDIA_API_KEY = 'nvapi-env-fake-key-1234567890abcdef'
  const { config } = await loadFresh({})
  assert(config.needsActivation === false, 'H: NVIDIA_API_KEY env 可激活')
  assert(config.provider === 'nvidia', 'H: env provider 为 NVIDIA')
  assert(config.model === 'openai/gpt-oss-120b', 'H: env 默认模型正确')
